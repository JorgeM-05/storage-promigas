...
public class FclRepositorylmp extends AbstractRepositoryDatabase implements FclRepository {

    private static String QUERY = "";

    private static String INSERT = "insert into dbo.cf_fcl values(?,?,?,?)";
    private static String DELETE = "delete from dbo.cf_fcl where id_opportunity = ?";
    ...
    public boolean insertFcl(int idOpportunity, FclEntity fclEntity, ConnectionInfo connectionInfo) {
        ...
        try {
            ...
            con.setString(2,fclEntity.getYear());
            con.setDouble(3,fclEntity.getFclUsd());
            con.setDouble(4,fclEntity.getFclCop());
    ...
    public List<Integer> getID(int idOportunity) throws SQLException {
        List<Integer> id= Collections.singletonList(0);
        QUERY = "select * from dbo.cf_fcl where id_opportunity = "+idOportunity;
